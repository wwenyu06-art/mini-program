...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  Object.assign(probe.style, {
    ...
    inset: '0 auto auto 0', fontFamily: props.fontFamily,
    fontSize: getFontValue(props.fontSize), fontWeight: String(props.fontWeight),
...
export default function WarpText({
  ...
  fontSize = 'clamp(3rem, 10vw, 9rem)', fontWeight = 800,
  ...
  useEffect(() => {
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uTime: { value: 0 }, uWarpStrength: { value: propsRef.current.warpStrength },
        uWarpScale: { value: propsRef.current.warpScale }, uSpeed: { value: propsRef.current.speed },
        ...
        uRipple: { value: propsRef.current.ripple ? 1 : 0 }, uMotion: { value: reduceMotion ? 0 : 1 }
      }
    });
    ...
    const onContextLost = event => { event.preventDefault(); contextLost = true; if (raf) cancelAnimationFrame(raf); raf = 0; };
    const loop = now => {
      if (disposed || contextLost) return;
    ...
    const onReducedMotion = event => { reduceMotion = event.matches; program.uniforms.uMotion.value = reduceMotion ? 0 : 1; renderOnce(); };
  ...
  return <span ref={containerRef} className={`warp-text ${className}`.trim()} style={style} role="img" aria-label={text} />;
}
